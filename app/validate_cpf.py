...
class Validation(Cpf):
    ...
    def validate_user_cpf(self):
        first = self.first_digit_cpf()
        second = self.second_digit_cpf()
        if self.cpf[-2:] == f'{first}{second}':
            return f'The cpf {self.cpf} is valid!'
        else:
            return f'The cpf {self.cpf} is invalid!' 